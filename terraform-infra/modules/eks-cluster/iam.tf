...
resource "aws_iam_role" "eks_cluster_role" {
  name = "${var.cluster_name}-cluster-role-final"
...
resource "aws_iam_role" "eks_node_role" {
  name = "${var.cluster_name}-worker-role-final"
  ...
  assume_role_policy = jsonencode({
    ...
    Statement = [{
      Action = "sts:AssumeRole"
      Effect = "Allow"
      Principal = { Service = "ec2.amazonaws.com" } # Crucial: Must trust EC2
    }]
  })
}

# Policies required for Worker Nodes to function
...
# Add permissions for the EBS CSI Driver to manage storage
